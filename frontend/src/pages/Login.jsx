...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    const roleRequested = activeTab === "siswa" ? "siswa" : "";
    const result = await login(username.trim(), password.trim(), roleRequested);
  ...
  const handleQuickLogin = async (userDemo, passDemo, tab) => {
    setActiveTab(tab);
    setUsername(userDemo);
    setPassword(passDemo);
    setErrorMessage("");

    const roleRequested = tab === "siswa" ? "siswa" : "";
    const result = await login(userDemo, passDemo, roleRequested);
    if (result.success) {
      if (result.user.role === "siswa") navigate("/portal-siswa");
      else if (result.user.role === "piket") navigate("/portal-piket");
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-200/70">
          ...
          <div className="mt-6 pt-5 border-t border-slate-100">
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                type="button"
                onClick={() => handleQuickLogin("admin", "admin123", "staf")}
                className="p-2 rounded-xl bg-slate-50 hover:bg-blue-50 hover:text-blue-700 border border-slate-200 text-slate-700 text-[11px] font-semibold text-center transition-all cursor-pointer"
              >
                Admin
              </button>
              <button
                type="button"
                onClick={() => handleQuickLogin("piket", "piket123", "staf")}
                className="p-2 rounded-xl bg-slate-50 hover:bg-emerald-50 hover:text-emerald-700 border border-slate-200 text-slate-700 text-[11px] font-semibold text-center transition-all cursor-pointer"
              >
                Guru Piket
              </button>
              <button
                type="button"
                onClick={() => handleQuickLogin("21312", "21312", "siswa")}
                className="p-2 rounded-xl bg-slate-50 hover:bg-purple-50 hover:text-purple-700 border border-slate-200 text-slate-700 text-[11px] font-semibold text-center transition-all cursor-pointer"
              >
                Siswa (21312)
              </button>
            </div>
